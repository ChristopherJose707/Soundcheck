...
class SongShow extends React.Component {
        ...
        >
          <div className="comment-photo-div">
            <Link to={`/users/${comment.author_id}`}>
              {this.props.users[comment.author_id].profilePicture ? (
                <img
  ...
  render() {
    ...
    const songPhoto = song.songPhoto ? (
      <img className="song-show-photo" src={song.songPhoto} />
    ) : null;
    ...
    const uploadPhotoButton =
      song.artist !== currentUser.display_name ? null : !song.songPhoto &&
        song.artist === currentUser.display_name ? (
    ...
    const deleteButton =
      song.artist !== currentUser.display_name ? null : (
        <button className="song-delete" onClick={() => this.handleDelete()}>
          Delete Track
        </button>
      );
          ...
          <div className="song-banner-info">
            <div className="song-banner-top">
              <h2 className="song-banner-artist">
                <Link to={`/users/${song.user_id}`}>{song.artist}</Link>
              </h2>
              <h3 className="song-banner-created-at">
                {uploadTime(song.created_at)}
              </h3>
            </div>
